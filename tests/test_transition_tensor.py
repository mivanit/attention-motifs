...
def test_transition_tensor_implementations() -> None:
    ...
    A_torch: Float[torch.Tensor, "n_ctx n_ctx"] = torch.tensor(A_np, dtype=torch.float32)
    ...
    idxs_torch, tt_torch, _ = transition_tensor_torch(
        A_torch, exact=exact, approx_l10=approx_l10, approx_pts=approx_pts, residuals=False
    )
    ...
    torch_indices = np.array([np.where(idxs_torch_np == idx)[0][0] for idx in common_indices])
    ...
    rel_diff = np.max(np.abs(tt_np_common - tt_torch_np_common) / (np.abs(tt_np_common) + 1e-10))
    
    assert max_abs_diff < 1e-5, \
        f"Max absolute difference too large: {max_abs_diff}"
    assert rel_diff < 1e-4, \
        f"Max relative difference too large: {rel_diff}"
