...
class TestBuildDistanceTensorHelper:
	...
	def test_reduce_true_shape(self) -> None:
		...
		result: Float[np.ndarray, "h h"] = _build_distance_tensor(data, reduce=True)
	...
	def test_reduce_false_shape(self) -> None:
		...
		rng: np.random.Generator = np.random.default_rng(0)
		data: Float[np.ndarray, "p h d"] = rng.standard_normal((8, 4, 3))
		result: Float[np.ndarray, "h h p"] = _build_distance_tensor(data, reduce=False)
	...
	def test_reduce_matches_full_mean(self) -> None:
		...
		reduced: Float[np.ndarray, "h h"] = _build_distance_tensor(data, reduce=True)
		full: Float[np.ndarray, "h h p"] = _build_distance_tensor(data, reduce=False)
...
class TestBuildDistanceTensor:
	...
	def test_serial_reduced(self) -> None:
		...
		result: DistanceTensorResult = DistanceTensorResult.build_distance_tensor(
			df, reduce=True
		)
	...
	def test_parallel_requires_reduce(self) -> None:
		...
		with pytest.raises(AssertionError, match="parallel mode requires reduce"):
			DistanceTensorResult.build_distance_tensor(df, parallel=True, reduce=False)
...
class TestEquivalence:
	...
	def test_serial_reduced_matches_full_mean(self, df: pl.DataFrame) -> None:
		"""Serial reduce=True matches reduce=False .mean_dists."""
		full: DistanceTensorResult = DistanceTensorResult.build_distance_tensor(
			df, reduce=False
		)
		reduced: DistanceTensorResult = DistanceTensorResult.build_distance_tensor(
			df, reduce=True
		)
		np.testing.assert_allclose(reduced.distances, full.mean_dists, rtol=1e-12)

	def test_parallel_matches_serial(self, df: pl.DataFrame) -> None:
		"""Parallel n_proc=2 matches serial reduce=True."""
		serial: DistanceTensorResult = DistanceTensorResult.build_distance_tensor(
			df, reduce=True
		)
		parallel: DistanceTensorResult = DistanceTensorResult.build_distance_tensor(
			df, parallel=True, n_proc=2
		)
		np.testing.assert_allclose(parallel.distances, serial.distances, rtol=1e-12)

	def test_parallel_nproc_1_matches_serial(self, df: pl.DataFrame) -> None:
		"""Parallel with n_proc=1 (single-process fast path) matches serial."""
		serial: DistanceTensorResult = DistanceTensorResult.build_distance_tensor(
			df, reduce=True
		)
		parallel_1: DistanceTensorResult = DistanceTensorResult.build_distance_tensor(
			df, parallel=True, n_proc=1
		)
		np.testing.assert_allclose(parallel_1.distances, serial.distances, rtol=1e-12)

	def test_parallel_nproc_4_matches_serial(self, df: pl.DataFrame) -> None:
		"""Parallel with n_proc=4 matches serial."""
		serial: DistanceTensorResult = DistanceTensorResult.build_distance_tensor(
			df, reduce=True
		)
		parallel_4: DistanceTensorResult = DistanceTensorResult.build_distance_tensor(
			df, parallel=True, n_proc=4
		)
		np.testing.assert_allclose(parallel_4.distances, serial.distances, rtol=1e-12)
...
class TestProperties:
	...
	def test_symmetric(self, df: pl.DataFrame) -> None:
		...
		np.testing.assert_allclose(result.distances, result.distances.T, atol=1e-14)

	def test_diagonal_zero(self, df: pl.DataFrame) -> None:
		result: DistanceTensorResult = DistanceTensorResult.build_distance_tensor(
			df, reduce=True
		)
		np.testing.assert_allclose(np.diag(result.distances), 0.0, atol=1e-14)
	...
	def test_cls_values_match_data(self, df: pl.DataFrame) -> None:
		...
		expected_cls: set[str] = set(df.get_column("activation.cls").unique().to_list())
		assert set(result.cls_values) == expected_cls

	def test_mean_dists_property_consistent(self, df: pl.DataFrame) -> None:
