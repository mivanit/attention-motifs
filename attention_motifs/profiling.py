...
class MemoryStats:
    ...
    def gpu_peak_mb(self) -> Optional[float]:
        ...
        gpu_mems = [s.gpu_allocated_mb for s in self.snapshots if s.gpu_allocated_mb is not None]
    ...
    def summarize(self) -> str:
        ...
        gpu_mbs = [s.gpu_allocated_mb for s in self.snapshots if s.gpu_allocated_mb is not None]
        if gpu_mbs:
            lines.append("\nGPU Memory (MB):")
    ...
    @staticmethod
    def _percentile_stats(values: Sequence[float], percentiles: Sequence[int] = (0, 25, 50, 75, 100)) -> list[str]:
...
class TrainingProfiler:
    ...
    def __init__(
        self,
        mode: ProfilerMode = ProfilerMode.FULL,
    ...
    def profile(self, name: str) -> Iterator[None]:
        ...
        cpu_profiler = cProfile.Profile() if self.mode in (ProfilerMode.TIME, ProfilerMode.FULL) else None
        if cpu_profiler:
            cpu_profiler.enable()
        ...
        gpu_profiler_ctx = (
            ...
            if torch.cuda.is_available() and self.mode in (ProfilerMode.TIME, ProfilerMode.FULL)
            else nullcontext()
        )
