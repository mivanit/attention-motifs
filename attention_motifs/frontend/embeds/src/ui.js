...
class UIManager {
  constructor(pointCloud) {
    ...
    this.uiConfig = {
      help: {
        ...
        elementId: "helpMenu",
        shortcutText: "h – help",
        visible: CONFIG.panels.help,
      },
      menu: {
        key: "KeyM",
        elementId: "controlsMenu",
        shortcutText: "m – menu",
        visible: CONFIG.panels.menu,
      },
      info: {
        key: "KeyI",
        elementId: "infoMenu",
        shortcutText: "i – info",
        visible: CONFIG.panels.info,
      },
      legend: {
        key: "KeyL",
        elementId: "legendMenu",
        shortcutText: "l – legend",
        visible: CONFIG.panels.legend,
      },
      navbar: {
        key: "KeyN",
        elementId: "navbar",
        shortcutText: "n – navball",
        visible: CONFIG.panels.navbar,
      },
      stats: {
        key: "KeyJ",
        elementId: "statsMenu",
        shortcutText: "j – stats",
        visible: CONFIG.panels.stats,
      },
    };

    /* categorical columns for c / v cycling ------------------------ */
    this.cats = this.pointCloud.model.df.columns.filter(
      (c) => !c.startsWith(CONFIG.numericalPrefix),
    );
    this.colorIdx = Math.max(
      0,
      this.cats.indexOf(this.pointCloud.state.colorBy),
    );
    this.selectIdx = Math.max(
      0,
      this.cats.indexOf(this.pointCloud.state.selectBy),
    );
  ...
  _applyInitialPanelVisibility() {
    for (const [name, cfg] of Object.entries(this.uiConfig)) {
      document.getElementById(cfg.elementId).style.display = cfg.visible
        ? "block"
        : "none";
    }
  }
  ...
  _setupNavball() {
    ...
    document.getElementById("navbar").style.display = CONFIG.panels.navbar
      ? "block"
      : "none";
    document.getElementById("legendMenu").style.display = CONFIG.panels.legend
      ? "block"
      : "none";
  }
  ...
  _buildShortcutsLegend() {
    ...
    sc.innerHTML =
      "<div>mouse - look around (2x click!)</div><div>wasd - move</div>";
    ...
    sc.addEventListener("click", (e) => {
      ...
      if (action === "hover-toggle") {
        ...
        this._updateStatusIndicator(
          "hover-status",
          CONFIG.interaction.hoverActive,
        );
        if (!CONFIG.interaction.hoverActive)
        ...
        this._updateStatusIndicator(
          "click-select-status",
          CONFIG.interaction.selectOnClick,
        );
        updateURL(); // Sync to URL
      } else if (action === "right-click-toggle") {
        CONFIG.interaction.rightClickActive =
          !CONFIG.interaction.rightClickActive;
        ...
        const entry = Object.entries(this.uiConfig).find(
          ([, cfg]) => cfg.elementId === action,
        );
        if (entry) {
          this._togglePanel(entry[0]);
          this._updatePanelStatusIndicator(
            target,
            this.uiConfig[entry[0]].visible,
          );
        }
      }
    });
  ...
  _setupControlSliders() {
    ...
    const nonSelPointSizeValue = document.getElementById(
      "nonSelPointSizeValue",
    );
    ...
    if (pointSizeSlider) {
      ...
      if (pointSizeValue)
        pointSizeValue.textContent = CONFIG.selectedPoints.size;
    }
    ...
    if (nonSelPointSizeSlider) {
      ...
      if (nonSelPointSizeValue)
        nonSelPointSizeValue.textContent = CONFIG.nonSelectedPoints.size;
    }
    ...
    if (opacitySlider) {
      ...
      if (opacityValue)
        opacityValue.textContent = CONFIG.selectedPoints.opacity.toFixed(2);
    }
    ...
    if (nonSelOpacitySlider) {
      ...
      if (nonSelOpacityValue)
        nonSelOpacityValue.textContent =
          CONFIG.nonSelectedPoints.opacity.toFixed(2);
    }
    ...
    if (nonSelColorPicker) {
      nonSelColorPicker.addEventListener("input", () => {
        this.pointCloud.state.setVisParam(
          "nonSelColor",
          nonSelColorPicker.value,
        );
      });
    }
    ...
    if (applyAxes) {
      applyAxes.addEventListener("click", async () => {
        ...
        try {
          ...
          const changes = [];
          if (xChanged)
            changes.push(`X: ${this.pointCloud.model.numericCols[newX]}`);
          if (yChanged)
            changes.push(`Y: ${this.pointCloud.model.numericCols[newY]}`);
          if (zChanged)
            changes.push(`Z: ${this.pointCloud.model.numericCols[newZ]}`);
  ...
  _updateLegendDisplay() {
    ...
    if (this.pointCloud.state.isNumericColumn(colorColumn)) {
      // Show colorbar info
      const values = this.pointCloud.model.df
        .col(colorColumn)
