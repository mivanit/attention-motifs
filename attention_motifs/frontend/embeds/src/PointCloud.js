...
class PointCloud {
  ...
  constructor(model) {
    ...
    this.raycaster.params.Points = {
      threshold: CONFIG.interaction.raycastThreshold,
    };
    ...
    for (const [id] of this.touchState.touches) {
      if (!activeTouchIds.has(id)) {
        ...
        if (
          ...
          if (
            duration < this.touchState.tapThreshold &&
  ...
  _applyTouchPan(deltaX, deltaY) {
    ...
    if (pitchDelta) {
      const newPitch = Math.max(
        -Math.PI / 2,
        Math.min(Math.PI / 2, this.pitch + pitchDelta),
      );
  ...
  _openContentTab(templateData) {
    const title = this._replaceTemplate(
      CONFIG.rightClick.content.title,
      templateData,
    );
    const content = this._replaceTemplate(
      CONFIG.rightClick.content.template,
      templateData,
    );
...
<body>${this._escapeHtml(content)}</body>
  ...
  _openUrlTab(templateData) {
    const url = this._replaceTemplate(
      CONFIG.rightClick.url.template,
      templateData,
    );
  ...
  handleSettingChange(prop) {
    if (prop === "pointSize" || prop === "opacity") {
      ...
      this.raycaster.params.Points.threshold =
        this.settings.pointSize * CONFIG.interaction.raycastThresholdMultiplier;
    }
  }
  ...
  _moveCamera() {
    ...
    if (this.velocity.lengthSq()) {
      const speed =
        this.settings.speed *
        (this.keys["ShiftLeft"] ? CONFIG.movement.sprintMultiplier : 1) *
        0.016;
      this.velocity
        .normalize()
