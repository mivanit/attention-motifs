    ...
    try {
      ...
      if (filename === null || filename === undefined) {
        if (!CONFIG.data || !Array.isArray(CONFIG.data)) {
          const errorMsg =
            "No dataFile specified and no embedded data found in CONFIG.data";
          spinner.complete();
        ...
        if (fileExtension === "csv") {
          ...
          console.warn(
            `Unknown file extension: .${fileExtension}, defaulting to JSONL parsing`,
          );
          df = DataFrame.from_jsonl(text);
        }
      ...
      const numeric = df.columns
        ...
        .sort((a, b) => {
          ...
          if (
            !isNaN(aNum) &&
            !isNaN(bNum) &&
            aNum.toString() === aSuffix &&
            bNum.toString() === bSuffix
          ) {
            return aNum - bNum;
          }
      ...
      // Complete progress bar and show success message
      if (pbar) {
        pbar.progress(1.0);
        pbar.complete();
      }
      ...
      NOTIF.success(
        `Loaded ${df.data.length} data points with ${numeric.length} dimensions`,
      );
