...
class ExperimentResults:
    ...
    def load(cls, path: Path | str) -> "ExperimentResults":
        ...
        config = ExperimentConfig(
            ...
            ablation_methods=[AblationMethod(m) for m in data["config"]["ablation_methods"]],
...
def run_ablation_experiment(
    ...
    if candidate_heads and isinstance(candidate_heads[0], str):
        candidate_heads = heads_from_strings(candidate_heads)

    if control_heads and isinstance(control_heads[0], str):
        control_heads = heads_from_strings(control_heads)
    ...
    if AblationMethod.MEAN in config.ablation_methods:
        ...
        calibration_tokens = [s.tokens for s in sequences[: config.n_calibration_prompts]]
        ablator.compute_mean_activations(calibration_tokens, show_progress=show_progress)
...
def run_cross_model_experiment(
    ...
    for model_name in models:
        ...
        model_candidates = candidates.get_top_candidates(model_name, n_candidates_per_model)
