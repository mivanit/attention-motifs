    ...
    for batch_idx, seq in enumerate(sequences):
        ...
        for rep_idx in range(1, seq.n_repetitions):
            ...
            for offset in range(1, base_len):
                ...
                key_pos = prev_start + offset - 1  # -1 because we want the token that precedes

                if query_pos < attn_pattern.shape[1] and key_pos < attn_pattern.shape[2]:
        ...
        for rep_idx in range(1, seq.n_repetitions):
            ...
            for offset in range(base_len - 1):
                ...
                if pos < logit_contribution.shape[1]:
                    logit_increase = logit_contribution[batch_idx, pos, next_token].item()
