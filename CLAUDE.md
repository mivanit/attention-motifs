...
| `uv run python attention_motifs/pipeline/s3_feat_proc.py --add-metadata [dir]` | Fast-add `activation.model_family`/`activation.model_size` columns to `pca_web.csv` in `dir` (default: `data/features/`) |
