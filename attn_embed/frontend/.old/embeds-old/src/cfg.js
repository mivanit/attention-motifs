  ...
  Object.keys(config).forEach((key) => {
    ...
    if (
      ...
      Object.keys(configValue).forEach((subKey) => {
        logger.log(
          `Applying config.dataConfig.${subKey} = ${JSON.stringify(configValue[subKey])}`,
        );
    ...
    // Handle special case for pcaAxes object
      ...
      Object.keys(configValue).forEach((axisKey) => {
        logger.log(
          `Applying config.pcaAxes.${axisKey} = ${configValue[axisKey]}`,
        );
    ...
    // Apply other top-level properties directly
      ...
      logger.log(
        `Applying config.hoverColumns = ${JSON.stringify(configValue)}`,
      );
