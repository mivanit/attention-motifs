...
function getCurrentCameraPosition(plotContainer) {
  if (plotContainer && plotContainer._fullLayout) {
    try {
      ...
      return JSON.parse(JSON.stringify(plotContainer._fullLayout.scene.camera));
    } catch (e) {
 ...
 * @returns {Array} - Array of trace objects for Plotly
 */
function createTracesWithSelection(
  plotData,
  selectedColumn,
  selectedValues,
  options,
  ...
  console.log(
    `Selection counts: ${Object.keys(selectedIndices).length} selected values, ${nonSelectedIndices.length} non-selected points`,
  );
  ...
  if (nonSelectedIndices.length > 0) {
    console.log(
      `Creating trace for ${nonSelectedIndices.length} non-selected points...`,
    );
  ...
  selectedValuesArray.forEach((value, index) => {
    ...
    console.log(
      `Creating trace for selected value "${value}" with ${indices.length} points...`,
    );
