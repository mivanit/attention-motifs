...
function initializeFilters() {
  ...
  const uniqueDimensions = [
    ...new Set(allPlots.map((p) => p.n_components)),
  ].sort();
  const uniqueNeighbors = [
    ...new Set(allPlots.map((p) => parseInt(p.n_neighbors))),
  ].sort((a, b) => a - b);
  ...
  html += '<div class="filter-group">';
  html +=
    '<label class="filter-label" style="cursor: pointer;" onclick="toggleNeighborCheckboxes()">Number of Neighbors:</label>';
  html += '<div class="checkbox-group">';
...
function applyFilters() {
  ...
  const filteredPlots = allPlots.filter((plot) => {
    ...
    return (
      filters.methods.includes(plot.method) &&
      plot.n_components === "2" &&
...
function displayEmbeddingPlots(plots) {
  ...
  if (plots.length === 0) {
    container.innerHTML =
      '<p style="text-align: center; color: #666; padding: 40px;">No visualizations match the selected filters.</p>';
    return;
  }
  ...
  const methodGroups = {};
  plots
    .filter((plot) => plot.n_components === "2")
    .forEach((plot) => {
